...
function extractFeatures(history) {
    ...
    // Tối ưu tính toán & TITAN TIER XOR Matrix cho Cầu Đảo
    ...
    const bitwiseTx = last10.map(t => t === 'T' ? 1 : 0);
    let xorSum = 0;
    for(let i = 0; i < bitwiseTx.length - 1; i++) { xorSum ^= (bitwiseTx[i] ^ bitwiseTx[i+1]); }
    const isReversal = (xorSum === 1 && entropy(last10) > 0.85);
...
// 7. ADAPTIVE BRIDGE BREAKER - AI bẻ cầu thông minh (TITAN TIER)
function algoG_SuperBridgePredictor(history) {
    ...
    // 4. TITAN TIER - Phát hiện Cầu Đảo (Reversal) siêu chuẩn xác bằng XOR Matrix
    if (!prediction && features.isReversal) {
        prediction = lastRun.val === 'T' ? 'X' : 'T';
        confidence = 0.88; // Độ tin cậy VIP
    }
...
// --- API SERVER ---
const app = fastify({ logger: false }); // Tắt logger rác của Fastify để giao diện sạch sẽ
...
app.get("/", async () => { 
    return {
        status: "ok",
        msg: "Vi Long - Mít AI - Titan Tier (No Random)",
        version: "3.0 VIP",
        algorithms: ALL_ALGS.length,
        pattern_recognition: "Ma Trận XOR phân tích đảo cầu sâu (15+ mẫu)",
        endpoints: [
            "/api/taixiumd5/lc79",
...
`;
    ...
    console.log("\n🚀 Vi Long - Mít AI Titan Tier đã khởi động!");
    ...
    console.log("\n🔧 Hệ thống AI Pattern Master với 10 thuật toán VIP:");
    ALL_ALGS.forEach((alg, i) => console.log(`   ${i+1}. ${alg.id}`));
    console.log("\n🎯 Nhận diện 15+ mẫu cầu phức tạp Titan (No Random):");
    console.log("   • Cầu 1-1, 2-2, 3-3, 4-4");
    console.log("   • Cầu 2-1-2, 1-2-1, 3-2-3, 4-2-4");
    console.log("   • Cầu bệt dài, cầu ngắn, phân tích cầu đảo qua XOR Matrix");
    console.log("   • Mít AI phân tích tự động, không rác console");
};
